...
const state = { project: null, rows: [], scripts: [], approvedScript: null, route: "avatar", media: [], busy: false };
const modes = {
...
const details = {
  ...
  edit: ["Bước 08 · Finance Editorial Edit", "Tạo phiếu edit và feedback phiên bản", "Phiếu EDIT_REQUEST.md là hợp đồng đầu vào cho skill finance-editorial-edit trong project."],
...
async function api(path, options = {}) {
  const headers = {"X-CFD-Agent": "1", ...(options.headers || {})};
...
async function ensureProject(title = "Video CFD Finance", selectedVideo = null) {
  if (state.project) return state.project;
  const data = await api("/api/projects", {method:"POST", body:{title, selected_video:selectedVideo}});
...
function renderWorkbench(key) {
  ...
  box.innerHTML = workShell(item[0], item[1], item[2], `<div class="callout">Bước này được thực thi bởi skill <strong>finance-editorial-edit</strong> sau khi phiếu edit được tạo. Artifact và trạng thái vẫn được lưu theo project.</div>`);
}
...
function renderResearch(box) {
  box.innerHTML = workShell("Thao tác bước 01", "Research đối thủ", "Demo dùng dữ liệu mẫu; Live quét các URL kênh công khai bạn cung cấp.", `
    <div class="toolbar">
      ...
      <label class="field-label">Ngành/ngách<select class="field" id="researchIndustry"><option value="cfd-finance">CFD & Finance</option><option value="forex">Forex</option><option value="gold-xauusd">Vàng · XAUUSD</option><option value="stocks">Chứng khoán</option><option value="crypto">Crypto</option><option value="risk-management">Tâm lý · Quản trị rủi ro</option></select></label>
...
async function approveVideo(index) {
  ...
  await withBusy(null, "", async () => {
    ...
    const result = await api("/api/scripts/generate", {method:"POST", body:{project_id:project.id, selected_video:selected, mode}});
...
function renderScripts(box) {
  const cards = state.scripts.length ? `<div class="script-grid">${state.scripts.map((s, i) => scriptCard(s, i)).join("")}</div>` : `
    ...
    <div class="field-grid" style="margin-top:14px"><label class="field-label">Tên video<input class="field" id="manualTitle" value="Video CFD Finance"></label><label class="field-label">Kịch bản<textarea id="manualScript" style="min-height:170px" placeholder="Dán kịch bản đã có vào đây…"></textarea></label></div>
...
function scriptCard(script, index) {
  ...
  return `<article class="script-card" id="scriptCard${index}"><h4>Kịch bản ${index + 1} · ${esc(script.angle)}</h4><div class="score-row">${Object.entries(analysis).map(([k,v]) => `<span class="score"><strong>${esc(k)}:</strong> ${esc(v)}</span>`).join("")}</div><textarea aria-label="Kịch bản ${index + 1}">${esc(script.text)}</textarea><button class="mini-btn approve" style="width:100%;margin-top:9px" data-approve-script="${index}">Duyệt kịch bản ${index + 1}</button></article>`;
}
...
async function approveManualScript(event) {
  ...
  await withBusy(event.currentTarget, "Đang lưu…", async () => {
    const project = await ensureProject(document.getElementById("manualTitle").value.trim() || "Video CFD Finance");
...
function renderProduction(box) {
  ...
  box.innerHTML = workShell("Thao tác bước 04", "Chọn luồng tạo video gốc", ready ? "Kịch bản đã duyệt sẵn sàng sản xuất." : "Hãy duyệt kịch bản trước khi tạo video.", `
    ...
    <div id="nofacePane" class="hidden-pane"><div class="field-grid"><label class="field-label">Nguồn voice<select class="field" id="voiceSource"><option value="tts">AI voice / TTS</option><option value="upload">Voice thu âm</option><option value="text-music">Text + nhạc</option></select></label><label class="field-label">Phong cách<select class="field" id="nofaceStyle"><option value="finance-editorial">Finance editorial motion graphic</option><option value="market-news">Tin thị trường</option><option value="chart-lesson">Chart lesson</option></select></label></div><div class="visual-mix"><div class="visual-type"><b>Technical proof · 50%</b>Chart, vùng giá, cấu trúc.</div><div class="visual-type"><b>Real-world · 30%</b>Sự kiện và tổ chức thật.</div><div class="visual-type"><b>Stock · 20%</b>Bối cảnh phụ trợ.</div></div><button class="mini-btn primary-action" id="runNoface" style="margin-top:14px" ${ready ? "" : "disabled"}>Tạo kế hoạch No-face</button></div>`);
...
function renderEdit(box) {
  ...
  box.innerHTML = workShell("Thao tác bước 08", "Edit bằng finance-editorial-edit", "Tạo phiếu edit, sau đó gửi feedback theo từng vòng; file final có thể xem hoặc tải ngay tại localhost.", `
...
async function createEditRequest(event) {
  ...
  await withBusy(event.currentTarget, "Đang tạo phiếu…", async () => {
    await api("/api/edit-request", {method:"POST", body:{project_id:state.project.id, route:state.route, feedback:document.getElementById("editFeedback").value.trim()}});
    setStage("edit", "active", "Đã tạo phiếu");
    document.getElementById("editStatus").textContent = "Đã tạo EDIT_REQUEST.md và edit_request.json trong project.";
    toast("Phiếu edit đã sẵn sàng cho skill finance-editorial-edit");
  });
}
