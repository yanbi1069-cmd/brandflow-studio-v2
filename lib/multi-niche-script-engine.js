...
function spokenTokenCount(candidate) {
  return [candidate?.hook, candidate?.body, candidate?.cta, candidate?.disclaimer]
    .filter(Boolean)
    .join(" ")
    .trim()
    .split(/\s+/)
...
export async function generateMultiNicheScript(input = {}, sessionKey = "") {
  ...
  const voiceSpeed = Math.max(0.5, Math.min(1.5, Number(input.voiceSpeed) || 1));
  const minSpokenTokens = Math.round(targetSeconds * 4 * voiceSpeed);
  const maxSpokenTokens = Math.round(targetSeconds * 5 * voiceSpeed);
  ...
  const user = `${buildScriptBrief(input)}\nThương hiệu: ${input.brandName || "Chưa đặt tên"}\nChủ đề tham chiếu: ${input.trendTitle || "Tự chọn góc phù hợp"}\nLý do video nổi bật: ${input.trendHook || ""}\nThời lượng mục tiêu: ${targetSeconds} giây ở tốc độ giọng ${voiceSpeed}x.\nYêu cầu: tạo đúng 3 hướng khác nhau gồm cảnh báo trực diện, phản trực giác và case study. Mỗi bản phải là kịch bản hoàn chỉnh có hook tối thiểu 12 ký tự, body tối thiểu 120 ký tự, CTA tối thiểu 8 ký tự. Tổng số từ của hook + body + CTA + disclaimer phải nằm trong khoảng ${minSpokenTokens}–${maxSpokenTokens} từ để khớp thời lượng; không được rút gọn thành dàn ý. Giữ một ý chính, lời nói tự nhiên, có visual proof và CTA mềm. Không trả \"...\" ở bất kỳ field nào.`;
  ...
  for (const model of [...new Set([primaryModel, fallbackModel])]) {
    try {
      ...
      if (candidateRows.some((item) => {
        const count = spokenTokenCount(item);
        return count < minSpokenTokens || count > maxSpokenTokens;
      })) throw new Error(`kịch bản không đạt khoảng ${minSpokenTokens}–${maxSpokenTokens} từ cho ${targetSeconds} giây`);
      rows = candidateRows;
      usedModel = model;
      break;
    } catch (error) {
  ...
  const candidates = rows.map((item,index)=>{
    ...
    candidate.timing={targetSeconds,voiceSpeed,spokenTokens:spokenTokenCount(candidate),minSpokenTokens,maxSpokenTokens};
    return candidate;
  });
